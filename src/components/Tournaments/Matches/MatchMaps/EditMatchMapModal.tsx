// EditMatchMapModal.tsx
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import { TextField } from '@mui/material';
import { useUpdateMatchMapMutation } from "../../../../api/matchMapSlice.ts";
...
export function EditMatchMapModal({ isOpen, onClose, matchMap }: EditMatchMapModalProps) {
    ...
    const { register, handleSubmit, formState: { errors } } = useForm<MatchMapFormInputs>({
        ...
        defaultValues: matchMap,
    });
    const [updateMatchMap] = useUpdateMatchMapMutation();

    const onSubmit = async (data: MatchMapFormInputs) => {
        try {
            await updateMatchMap({
                map : {
                    id : matchMap.id,
            ...
            onClose();
            toast.success('Match Map updated successfully!', {position: "bottom-left"})
        } catch (error) {
            // Handle the error...
        }
    };
